...
window.initializeQuizShell = function(index) {
    console.log("Global: Initializing Quiz Phase", window.currentPhase, "at index", index);
...
window.showPsychometricHistogram = function() {
    ...
    container.innerHTML = `
        <div class="p-8 bg-brand-navy rounded-3xl border border-slate-700 shadow-2xl text-center" style="background: #0F172A;">
                ...
                ${['Cognitive Synthesis', 'Pressure Threshold', 'Ambition Vector', 'Logic Architecture'].map(label => `
                    <div class="space-y-2">
                        ...
                        <div class="h-3 bg-slate-800 rounded-full overflow-hidden">
                            <div class="h-full bg-[#FF6B35] animate-pulse" style="width: ${Math.random() * 40 + 60}%"></div>
                        </div>
                    </div>`).join('')}
            </div>
...
window.showSystemicRiskCard = function() {
    const container = document.getElementById('dynamicQuizContent');
    container.innerHTML = `
        <div class="fixed inset-0 z-[10000] flex items-center justify-center p-6 bg-brand-navy/80 backdrop-blur-sm animate-fade-in" style="background: rgba(15, 23, 42, 0.8);">
...
window.showPhase1BridgeCard = function() {
    const container = document.getElementById('dynamicQuizContent');
    container.innerHTML = `
        <div class="bg-slate-50 border-2 border-[#FF6B35] rounded-3xl p-8 shadow-2xl animate-fade-in-up">
            ...
            <div class="space-y-4 mb-10">
                <div class="flex items-center gap-4 bg-white p-4 rounded-xl border border-slate-200"><div class="text-[#FF6B35] text-2xl font-black">01</div><div class="text-brand-navy font-bold">5-Minute Deep-Dive into Academic DNA</div></div>
                <div class="flex items-center gap-4 bg-white p-4 rounded-xl border border-slate-200"><div class="text-[#FF6B35] text-2xl font-black">02</div><div class="text-brand-navy font-bold">Instant Fitment Score for CBSE, ICSE, and IB</div></div>
                <div class="flex items-center gap-4 bg-white p-4 rounded-xl border border-slate-200"><div class="text-[#FF6B35] text-2xl font-black">03</div><div class="text-brand-navy font-bold">Actionable Roadmap to a 2040 Global Career</div></div>
            </div>
            <button onclick="window.startPhase1()" class="w-full bg-[#FF6B35] text-white py-5 rounded-full font-black text-xl shadow-xl hover:scale-105 transition-all mb-4">Analyze My Child’s Fitment Board →</button>
...
window.triggerDNAFinalization = function() {
    ...
    container.innerHTML = `
        <div class="min-h-screen w-full bg-brand-navy flex flex-col items-center justify-center p-8 relative overflow-hidden" style="background: #0F172A;">
    ...
    const throbInterval = setInterval(() => {
        bars.forEach(bar => { if (bar) bar.style.width = Math.floor(Math.random() * 80 + 10) + "%"; });
        if (++throbCount > 15) {
            clearInterval(throbInterval);
            const status = document.getElementById('finalStatus');
            if (status) {
                status.innerText = "DNA ALIGNMENT COMPLETE: PROFILE SECURED.";
                status.style.color = "#10B981";
            }
            bars.forEach((bar, i) => { if (bar) { bar.classList.remove('liquid-animate'); bar.classList.add('vibrant-green'); bar.style.width = [88, 72, 94, 65, 81][i] + "%"; } });
            setTimeout(() => {
                const glass = document.getElementById('finalGlass');
