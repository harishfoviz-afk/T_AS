...
// --- DATA MASTERS (Restored to 31 Questions) ---
const MASTER_DATA = { 
...
const questions = [
    ...
    // Phase 1: Academic Fit (Index 4-18)
...
function checkPaymentStatus() {
    ...
    if (params.get('razorpay_payment_id')) {
        const orderId = localStorage.getItem('aptskola_last_order_id');
        const saved = JSON.parse(localStorage.getItem(`aptskola_session_${orderId}`));
        if (saved) {
            answers = saved.answers; customerData = saved.customerData;
            hideAllSections();
            renderReportToBrowser().then(() => showInstantSuccessPage());
        }
    }
}
...
function validateInputs(e, p) { return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e) && /^[6-9]\d{9}$/.test(p); }
...
function renderQuestionContent(index) {
    ...
    if (index === 4 && answers.p0_q4 !== undefined && !answers.p0_done) {
        ...
        document.getElementById('dynamicQuizContent').innerHTML = `<div style="text-align:center;"><div class="spinner"></div><h2>Scanning DNA...</h2></div>`;
        setTimeout(renderLogicBridge, 2000);
        return;
    }
    
    if (index === 19 && !isSyncMatchMode) {
        // Post-Q15 Synthesis Logic
        document.getElementById('dynamicQuizContent').classList.add('hidden');
        ...
        setTimeout(() => {
            bridge.classList.remove('active'); bridge.classList.add('hidden');
            showDetailsPage();
        }, 3000);
        return;
    }

    if (index === 31 && isSyncMatchMode) { 
        // Sync Match Complete
        calculateSyncMatch(); 
        return; 
    }
    ...
    let qText = q.text || q.text_variants[customerData.childAge] || q.text_variants["5-10"];
    let qOptions = q.options || q.options_variants[customerData.childAge] || q.options_variants["5-10"];
    ...
    `;
    document.getElementById('dynamicQuizContent').classList.remove('hidden');
}
...
function transitionToPhase1() {
    ...
    const q2 = answers.p0_q2;
    const rotation = (q2 === 1) ? 135 : 45; 
    document.documentElement.style.setProperty('--target-rotation', `${rotation}deg`);
    ...
    setTimeout(() => {
        initializeQuizShell(4);
    }, 3000);
}

function startAuthorityBridge() {
    // Legacy fallback or explicit call
    document.getElementById('dynamicQuizContent').classList.add('hidden');
    const bridge = document.getElementById('authorityBridge');
    bridge.classList.remove('hidden'); bridge.classList.add('active');
    ...
    bridge.innerHTML = `
        <div class="bridge-container">
            <h2 style="font-size:1.2rem; color:#94A3B8; text-transform:uppercase;">Synthesis in Progress</h2>
            <div class="risk-meter-box"><div class="risk-meter-arc"></div><div class="risk-meter-fill"></div></div>
            <div class="risk-label">Analyzing DNA...</div>
            <p style="color:#CBD5E1; margin-top:20px;">Syncing Phase 0 data with child’s Behavioral DNA.</p>
        </div>`;
...
function showPricingAfterAuthority() {
    document.getElementById('authorityBridge').classList.add('hidden');
    showDetailsPage();
}
...
function selectPackage(pkg, price) {
    ...
    if (price === 599) {
        ...
        document.getElementById('proUpgradeModal').classList.add('active');
    } else {
        confirmSelection();
    }
}
...
function upgradeAndProceed() {
    ...
    selectedPrice = 999;
    document.getElementById('downgradeModal').classList.remove('active');
    document.getElementById('proUpgradeModal').classList.add('active');
}
...
function upgradeToProAndProceed() {
    ...
    selectedPrice = 1499;
    document.getElementById('proUpgradeModal').classList.remove('active');
    confirmSelection();
}
...
function redirectToRazorpay() {
    ...
    const options = {
        ...
        currency: "INR",
        name: "Apt Skola",
        description: `Scientific Board Report - ${selectedPackage}`,
        payment_capture: 1,
        handler: function (response) {
            localStorage.setItem('aptskola_last_payment_id', response.razorpay_payment_id);
            const orderId = localStorage.getItem('aptskola_last_order_id');
            const sessionData = JSON.parse(localStorage.getItem(`aptskola_session_${orderId}`));
            if (sessionData) {
...
function processSyncUpgrade() {
    const options = {
        ...
        currency: "INR",
        name: "Apt Skola",
        description: "Sync Match Upgrade",
        payment_capture: 1,
        handler: function (response) {
            alert("Upgrade successful! You can now proceed to Sync Check.");
        },
        theme: { color: "#FF6B35" }
    };
    const rzp = new Razorpay(options);
...
function calculateFullRecommendation(ansSet) {
    ...
    if (ansSet.q1 === 0) { scores["IB"] += 6; scores["ICSE"] += 4; } 
    if (ansSet.q3 === 0) { scores["CBSE"] += 20; }
    let results = Object.keys(scores).map(board => ({ name: board, score: scores[board], percentage: Math.min(Math.round((scores[board] / 20) * 95), 99) }));
...
async function renderReportToBrowser() {
    ...
    const data = MASTER_DATA[res.recommended.name.toLowerCase()] || MASTER_DATA['cbse'];
    let html = `
        <div class="report-card" style="background:#0F172A; color:white; text-align:center;">
            ...
            <div style="font-size:1.1rem; opacity:0.8;">Scientific Report</div>
        </div>
        <div class="report-card">
            ...
            <div style="margin-top:10px; padding:10px; background:#F8FAFC; border-radius:8px;">
                Board Match: <span style="color:#FF6B35; font-weight:bold;">${res.recommended.name}</span>
            </div>
        </div>`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (overlay && !new URLSearchParams(window.location.search).get('razorpay_payment_id')) {
