...
# Core Synchronized Funnel Logic
funnel_logic = """
...
async function downloadReport() {
    ...
    // ... rest of original original downloadReport logic ...
    // Note: Re-adding the class should happen in the finally block
    // I'll ensure the original function is updated in the re-integration
}
"""

# Re-integration logic
# Replace Razorpay success handler to call renderReportToBrowser first
orig_content = orig_content.replace(
    'console.log("Payment Successful. ID: " + response.razorpay_payment_id);',
    'console.log("Payment Successful. ID: " + response.razorpay_payment_id); window.renderReportToBrowser().then(() => window.showInstantSuccessPage());'
)

# Updating customerForm listener
orig_content = orig_content.replace(
    'localStorage.setItem(\'aptskola_last_order_id\', newOrderId);',
    'localStorage.setItem(\'aptskola_last_order_id\', newOrderId); window.triggerDNAFinalization();'
)

# Final Merge
final_script = funnel_logic + "\n" + orig_content
