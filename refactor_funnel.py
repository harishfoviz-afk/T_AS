...
# Extract Phase 1 questions from original (q1 to q15)
q_match = re.search(r'const questions = \[(.*?)\];', original_content, re.DOTALL)
if q_match:
    raw_questions = q_match.group(1)
    # This is tricky because of text_variants objects. 
    # Let's just assume we want the first 15 objects.
    # A better way is to split by }, { or similar, but nested braces make it hard for regex.
    # For now, I will manually define the structure to be safe.
    pass

# Logic for the new script.js
funnel_js = """
// --- GLOBAL STATE ---
window.phase0Complete = false;
let currentPhase = 0; // 0 = Phase 0, 1 = Phase 1
...
// --- QUESTIONS ---
const phase0Questions = """ + str(phase0_questions) + """;
...
window.renderQuestionContent = function(index) {
    ...
    if (currentPhase === 1 && index >= qList.length) {
        ...
        if(dPage) { 
            dPage.classList.remove('hidden'); 
            dPage.classList.add('active'); 
            dPage.style.display = 'flex'; 
        }
        return;
    }
    ...
    if (currentPhase === 0) {
        progressPercent = ((index + 1) / totalQ) * 100;
    } else {
        const qNum = index + 1;
        if (qNum <= 5) {
    ...
    quizContent.innerHTML = `
        <div class="progress-container mb-10">
            ...
            <div class="flex justify-between items-center text-xs font-bold text-slate-400 uppercase tracking-widest">
                ...
                <span>Question ${index + 1} / ${totalQ}</span>
            </div>
        </div>
...
window.showSystemicRiskCard = function() {
    const container = document.getElementById('dynamicQuizContent');
    container.innerHTML = `
        <div class="fixed inset-0 z-[10000] flex items-center justify-center p-6 bg-brand-navy/80 backdrop-blur-sm">
...
# Now extract the massive master data from original_content
# I will use a simple split and join to preserve the data without parsing it as python objects.

# Find the start of MASTER_DATA
data_part = original_content.split('const MASTER_DATA = {')[1].split('const questions = [')[0]
master_data_str = "const MASTER_DATA = {" + data_part

# Find the rest of the file logic (forms, payments, reports, jsPDF stuff)
# It usually starts after the questions array
rest_of_file = original_content.split('const questions = [')[1].split('];', 1)[1]

# Combine everything
final_script = funnel_js + "\n" + master_data_str + "\n" + rest_of_file

with open('script.js', 'w') as f:
    f.write(final_script)

# Final sed to fix the customerForm listener in the new combined file
